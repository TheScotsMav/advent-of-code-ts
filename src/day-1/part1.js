...
const searchingFor = ['1', '2', '3', '4', '5', '6', '7', '8', '9', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'];
...
for (let line of splitInput) {
	...
	for (let i = 0; i < line.length; i++) {
		for (let search of searchingFor) {
			if (line.substring(i, Math.min(i + 5, line.length)).startsWith(search)) {
				if (!first) {
					first = searchingFor.indexOf(search) % 9 + 1;
				}
				last = searchingFor.indexOf(search) % 9 + 1;
			}
		}
	}
	total += first * 10 + last

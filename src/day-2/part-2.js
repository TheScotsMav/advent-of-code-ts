...
for (let line of lineSplit) {
    ...
    const gameId = parseInt(firstPart.substring(5))
    const rounds = secondPart.split('; ');
    let highestRed = 0, highestGreen = 0, highestBlue = 0;
    for (let round of rounds) {
        const cubes = round.split(', ');
        for (let cube of cubes) {
            if (cube.includes('red')) {
                const number = parseInt(cube.substring(0, cube.indexOf('r')))
                if (number > highestRed) {
                    highestRed = number;
                }

            } else if (cube.includes('green')) {
                const number = parseInt(cube.substring(0, cube.indexOf('g')))
                if (number > highestGreen) {
                    highestGreen = number;
                }

            } else if (cube.includes('blue')) {
                const number = parseInt(cube.substring(0, cube.indexOf('b')))
